...
function startSimulacro(type) {
  ...
  if (type === 'avanzado') {
    ...
    Object.entries(breakdown).forEach(([area, count]) => {
      ...
      const shuffled = shuffleArr(areaQs);
  ...
  simulacroState = {
    type, questions: pool, totalQuestions: pool.length, answers: {},
    timeRemaining: config.time * 60, currentIndex: 0, startTime: Date.now(), finished: false
  };
...
function renderSimulacroPlaying() {
      ...
      <div class="question-card">
        ...
        <div class="options-list">
          ${q.options.map((opt, i) => {
            ...
            return `<div class="${cls}" onclick="selectSimulacroOption(${i})"><span class="option-letter">${'ABCD'[i]}</span><span>${opt}</span></div>`;
          }).join('')}
        </div>
      </div>
      ...
      <div class="question-nav">
        ...
        <div style="display:flex;gap:6px;flex-wrap:wrap;justify-content:center">
          ${state.questions.slice(Math.max(0, state.currentIndex - 4), Math.min(total, state.currentIndex + 5)).map((_, i) => {
            ...
            return `<button class="${btnClass}" onclick="goToSimulacroQuestion(${idx})" style="min-width:36px">${idx + 1}</button>`;
          }).join('')}
        </div>
  ...
  simulacroTimer = setInterval(() => {
    state.timeRemaining--;
    if (state.timeRemaining <= 0) { clearInterval(simulacroTimer); finishSimulacro(); return; }
...
function selectSimulacroOption(idx) {
  ...
  state.answers[state.questions[state.currentIndex].id] = idx;
  renderSimulacroPlaying();
}

function nextSimulacroQuestion() {
  const state = simulacroState;
  if (state && state.currentIndex < state.totalQuestions - 1) { state.currentIndex++; renderSimulacroPlaying(); }
}

function prevSimulacroQuestion() {
  const state = simulacroState;
  if (state && state.currentIndex > 0) { state.currentIndex--; renderSimulacroPlaying(); }
}

function goToSimulacroQuestion(idx) {
  const state = simulacroState;
  if (state && idx >= 0 && idx < state.totalQuestions) { state.currentIndex = idx; renderSimulacroPlaying(); }
}
...
function calculateResults(state) {
  ...
  const areaResults = {};
  EXAM_CONFIG.areas.forEach(a => { areaResults[a.id] = {total:0, correctas:0, incorrectas:0, sinResponder:0}; });
  state.questions.forEach(q => {
    const ans = state.answers[q.id];
    if (ans === undefined) { sinResponder++; if (areaResults[q.area]) areaResults[q.area].sinResponder++; }
    else if (ans === q.correct) { correctas++; if (areaResults[q.area]) { areaResults[q.area].correctas++; areaResults[q.area].total++; } }
    else { incorrectas++; if (areaResults[q.area]) { areaResults[q.area].incorrectas++; areaResults[q.area].total++; } }
  });
  let puntaje = 0;
  if (state.type === 'avanzado') {
    ...
    EXAM_CONFIG.areas.forEach(a => {
      ...
      if (areaTotal > 0) totalPonderado += (ar.correctas / areaTotal) * a.weight;
    });
    puntaje = Math.round(totalPonderado * 500);
  } else {
    const answered = correctas + incorrectas;
    puntaje = answered > 0 ? Math.round((correctas / answered) * 500) : 0;
  }
  puntaje = Math.min(500, Math.max(0, puntaje));
  const timeSpent = Math.round((Date.now() - state.startTime) / 1000);
  return {
    date: new Date().toISOString(), type: state.type, totalQuestions: total,
    correctas, incorrectas, sinResponder, puntaje, timeSpent, areaResults,
    answers: state.answers, questions: state.questions.map(q => ({id: q.id, correct: q.correct, area: q.area}))
  };
}

function renderSimulacroResults(results) {
  if (!results) { navigate('simulacro'); return; }
  const config = EXAM_CONFIG.simulacros[results.type];
  const total = results.totalQuestions;
  const {correctas, incorrectas, sinResponder} = results;
    ...
    <div class="container">
      <div class="results-container">
        <div class="results-header">
          ...
          <div style="font-size:13px;color:var(--text-light);margin-top:4px">${results.type === 'avanzado' ? 'Puntaje ponderado oficial' : 'Puntaje estimado'}</div>
        </div>

        <div class="results-details">
          <div class="result-item"><div class="value green">${correctas}</div><div class="label">Correctas</div></div>
          <div class="result-item"><div class="value red">${incorrectas}</div><div class="label">Incorrectas</div></div>
          <div class="result-item"><div class="value" style="color:#95a5a6">${sinResponder}</div><div class="label">Sin responder</div></div>
          <div class="result-item"><div class="value" style="color:var(--primary)">${total > 0 ? Math.round(((correctas + incorrectas) / total) * 100) : 0}%</div><div class="label">Completado</div></div>
          <div class="result-item"><div class="value" style="color:var(--primary)">${timeMins}:${String(timeSecs).padStart(2, '0')}</div><div class="label">Tiempo usado</div></div>
        </div>
        ...
        <div class="area-breakdown">
          ...
          ${EXAM_CONFIG.areas.map(a => {
            ...
            return `<div class="area-row"><span class="area-name" style="color:${a.color}">${a.icon} ${a.name}</span><div class="area-bar"><div class="area-bar-fill" style="width:${pctArea}%;background:${a.color}"></div></div><span class="area-score">${ar.correctas}/${totalArea} (${pctArea}%)</span></div>`;
          }).join('')}
        </div>
    ...
    html += `
      <div class="question-card" style="margin-bottom:12px;padding:16px">
        <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:8px">
          ...
          <span style="font-size:14px;font-weight:700;color:${isCorrect ? 'var(--success)' : 'var(--error)'}">${isCorrect ? '✅ Correcta' : '❌ Incorrecta'}</span>
        </div>
